...
export default function JasuBrainSection() {
  ...
  // Wrapper for the whole left card (used for styling only)
  const wrapRef = useRef<HTMLDivElement | null>(null);

  // IMPORTANT: Canvas should be sized to the square that contains the SVG rings
  ...
  useEffect(() => {
    ...
    // Radii are defined in pixels relative to the canvas center.
    // These are approximate and tuned to your 420 viewBox rings.
    const ringR = {
      userbrain: 70,
      neocortex: 105,
      limbic: 140,
    ...
    const spawn = (key: BrainLayerKey) => {
      ...
      const speed = key === "userbrain" ? rand(0.9, 1.6) : rand(0.5, 1.0);
      const vx = Math.cos(t + rand(-0.8, 0.8)) * speed;
      const vy = Math.sin(t + rand(-0.8, 0.8)) * speed;

      const max = key === "userbrain" ? rand(22, 34) : rand(16, 24);
    ...
    const step = () => {
      ...
      // Very subtle haze; keep it low so sparks feel crisp
      ctx.fillStyle = "rgba(255,255,255,0.015)";
      ctx.fillRect(0, 0, w, h);

      // Spawn rate responds to active layer
      const base = 2;
      const bonus = active === "userbrain" ? 4 : 3;
      const n = base + bonus;
    ...
    <Section tone="s7" className="py-24">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-10 lg:grid-cols-[520px_1fr] lg:items-start">
          ...
          <div
            ref={wrapRef}
            className="relative rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--bg))]/15 p-8 backdrop-blur"
          >
            {/* IMPORTANT: Canvas overlays ONLY the square that contains the rings */}
            <div
              ref={brainRef}
            ...
            <div className="mt-4 text-center text-xs text-[rgb(var(--muted))]">
              Hover or click a layer to explore
            </div>

            {/* Mini legend buttons (helps on touch/mobile) */}
            ...
            >
              ...
              <div className="mt-8 grid gap-6 md:grid-cols-3">
                <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg))]/15 p-4">
                  <div className="text-xs font-semibold tracking-[0.26em] text-[rgb(var(--muted))]">
                    LAYER
                  </div>
                ...
                <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg))]/15 p-4">
                  ...
                  <div className="mt-2 text-sm leading-relaxed text-[rgb(var(--text))]/85">
                    {activeLayer.right}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 text-sm leading-relaxed text-[rgb(var(--muted))]">
                Tip: Keep <span className="text-[rgb(var(--text))]">UserBrain</span>{" "}
                selected when pitching — it’s the “this doesn’t exist elsewhere” hook.
              </div>
            </motion.div>
          </div>
        </div>
    ...
    >
      ...
      <motion.circle
        ...
        animate={{
          ...
          filter: active
            ? `drop-shadow(0px 0px ${isGold ? 18 : 14}px ${
                isGold ? "rgba(245,158,11,0.45)" : "rgba(59,130,246,0.22)"
              })`
            : "none",
        }}
